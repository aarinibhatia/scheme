...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('length of formals and legnth of values do not match')
        else:
...
        check_form(expresssions, 2, 2)
        val = scheme_eval(expresssions.second.first, env)
        env.define(expressions.first, val)
        return expressions.first
    elif isinstance(expressions.first, Pair) and scheme_symbolp(expressions.first.first):
        env.define(expressions.first.first, apply_macro(expressions.first.second, expressions.second, env))
